...
export default function EventDetail() {
  ...
  if (loading) return <div className="text-center mt-20 text-gray-400">Loading event...</div>;
            ...
            <div key={ticket.id} className="border border-gray-700 rounded-xl p-6">
              <div className="flex justify-between items-center mb-4">
                <div>
                  ...
                  <p className="text-2xl font-bold">${ticket.price}<span className="text-sm text-gray-400">/seat</span></p>
                </div>
                      ...
                      {Array.from({ length: SEATS_PER_ROW }, (_, i) => i + 1).map(seat => {
                        ...
                        return (
                          <button key={seat} onClick={() => toggleSeat(ticket.id, row, seat)}
                            className={`w-6 h-6 rounded-t-md text-[9px] font-bold transition ${
                              isSelected ? `${colors[idx % 4]} text-white` : "bg-gray-800 hover:bg-gray-600 text-gray-500"
                            }`}
                            title={`${ticket.name} Row ${row} Seat ${seat} - $${ticket.price}`}
                          >
                            {seat}
                          </button>
                        );
